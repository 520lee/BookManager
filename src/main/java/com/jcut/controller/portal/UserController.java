...
public class UserController {
    ...
    public ServerResponse<User> login(String username,
                                      ...
                                      HttpSession session){
        //service - mybatis - dao
        ServerResponse<User> response = iUserService.login(username,
                password);
    ...
    @ResponseBody
    public ServerResponse<User> update_information(HttpSession session, User newUser){
        User currentUser = (User) session.getAttribute(Const.CURRENT_USER);
        if (currentUser == null){
            return ServerResponse.createByErrorMessage("用户未登录");
        }
        newUser.setId(currentUser.getId());
        ServerResponse<User> response = iUserService.updateInformation(currentUser, newUser);
        if (response.isSuccess()){
    ...
    public ServerResponse<User> get_information(HttpSession session){
        ...
        return iUserService.getinformation(user.getUsername());
    }
}
